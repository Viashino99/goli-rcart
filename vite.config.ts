...
export default defineConfig({
  ...
  build: {
    cssCodeSplit: false,
    sourcemap: false,
    // Inline fonts/images as data URIs so the IIFE is fully self-contained — theme-app-extension
    // assets are served from a Shopify CDN path the bundled import can't resolve, so an emitted
    // file would 404 on the storefront.
