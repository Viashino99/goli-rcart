...
/** API: id 1 = install / surprise gift ($25), id 2 = bundle goal ($150). Falls back to comparing targetAmount against the bundle threshold. */
...
export function RcartWidget({
  ...
  const generateCodeForTier = async (preferHighest: boolean): Promise<string | null> => {
    ...
    const discountAmount: number = tier === 'install' ? 25 : 150;
    ...
    if (!reused) {
      if (tier === 'install' && !notifyClaimInstallSentRef.current) {
        ...
        void callNotifyApiRef.current({
          id: 'goal-reached',
          icon: 'dollar',
          label: 'Bundle Goal Reached — $150 off',
          targetAmount: 150,
          status: 'claimed',
          discountCode: code,
        });
